...
struct CalendarGridView: View {
    ...
    var calendarGridView: some View {
        ...
        _ = numberOfRows * 7 - (daysInMonth + firstWeekday)
        
        return LazyVGrid(columns: Array(repeating: GridItem(), count: 7)) {
            ForEachDaysInMonth(daysInMonth: daysInMonth, firstWeekday: firstWeekday) { index in
                CalendarCellViewForRow(index: index, daysInMonth: daysInMonth, firstWeekday: firstWeekday, lastDayOfMonthBefore: lastDayOfMonthBefore, clickedCurrentMonthDates: self.$clickedCurrentMonthDates, getDate: self.getDate, calendarGridView: self.calendarGridView as! CalendarGridView)
                    .onTapGesture {
    ...
    struct CalendarCellViewForRow: View {
        ...
        let calendarGridView: CalendarGridView
        ...
        var body: some View {
            Group {
               if let prevMonthDate = Calendar.current.date(
                    ...
                    to: calendarGridView.previousMonth()
                ) {
                    let day = Calendar.current.component(.day, from: prevMonthDate)
                    CalendarCellView(day: day, isCurrentMonthDay: false, isSunday: true, weekday: (firstWeekday + index) % 7)
                }
            }
        }
    }
